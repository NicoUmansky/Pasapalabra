...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.error('')
  console.error('Puedes crear un archivo .env.local con:')
...
async function downloadQuestionsFromSupabase() {
  try {
    ...
    if (!data || data.length === 0) {
      console.warn('⚠️ No se encontraron preguntas en Supabase')
      return []
    }
...
async function syncQuestionsFromSupabase() {
  try {
    ...
    // Descargar preguntas desde Supabase
    const questions = await downloadQuestionsFromSupabase()
    
    if (questions.length > 0) {
      // Guardar en archivo local (sobrescribir si existe)
      await saveQuestionsToLocalFile(questions)
      console.log('✅ Sincronización completada exitosamente')
      console.log(`📊 Total de preguntas sincronizadas: ${questions.length}`)
      
      // Mostrar estadísticas por letra
      const stats = {}
      questions.forEach(q => {
        if (!stats[q.letter]) stats[q.letter] = 0
        stats[q.letter]++
      })
      
      console.log('\n📈 Estadísticas por letra:')
      Object.keys(stats).sort().forEach(letter => {
        console.log(`   ${letter}: ${stats[letter]} preguntas`)
      })
